...
public class Action106MessageParser extends Action103MessageParser {
    ...
    public String getMessagePreviewText(Message message) {
        Action106Builder builder = decodeContent(message);
        return (LvxinApplication.getInstance().getString(R.string.tip_agree_invitegroup, builder.name, builder.groupName));

    }


    @Override
    public Action106Builder decodeContent(Message message) {

        return new Gson().fromJson(message.content, Action106Builder.class);
    }
