...
public class Action102MessageHandler implements CustomMessageHandler {
    ...
    public boolean handle(Context context, Message message) {
        ...
        List<com.farsunset.lvxin.model.Message> list = MessageRepository.queryMessageByAction(Constant.MessageAction.ACTION_102);
        for (com.farsunset.lvxin.model.Message msg : list) {
            if (groupId.equals(new Gson().fromJson(msg.content, Action102Builder.class).groupId)
