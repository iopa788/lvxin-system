...
public class MediaObject implements Serializable {
     ...
     * 最终视频输出路径
     */
    private static final String TEMP_DIR_NAME = "temp_";
    /**
     * 主题
     */
    ...
    public MediaObject(String key, String path, int videoBitrate) {
        this.mKey = key;
        this.mOutputDirectory = path + File.separator + TEMP_DIR_NAME;
    ...
    private void makeTempDir() {
        File dir = new File(mOutputDirectory);
        if (!dir.exists()) {
            dir.mkdir();
        }
    }
    ...
    public void moveOutTempDir() {
        File mp4 = new File(mOutputVideoPath);
        File jpg = new File(mOutputVideoThumbPath);
        try {
            FileUtils.moveFileToDirectory(mp4, mp4.getParentFile().getParentFile(), true);
            FileUtils.moveFileToDirectory(jpg, jpg.getParentFile().getParentFile(), true);
        } catch (Exception e) {
        }
    }
    ...
    public String getOutputVideoName() {
        return new File(mOutputVideoPath).getName();
    }
    ...
    public long getOutputVideoSize() {
        return new File(mOutputVideoPath).length();
    }

    /**
     * 获取视频截图最终输出名字
     */
    public String getOutputVideoThumbName() {
        return new File(mOutputVideoThumbPath).getName();
    }
    ...
    public static class MediaPart implements Serializable {
        ...
        public void delete() {
            FileUtils.deleteQuietly(new File(mediaPath));
            FileUtils.deleteQuietly(new File(audioPath));
            FileUtils.deleteQuietly(new File(thumbPath));
            FileUtils.deleteQuietly(new File(tempMediaPath));
            FileUtils.deleteQuietly(new File(tempAudioPath));
        }
