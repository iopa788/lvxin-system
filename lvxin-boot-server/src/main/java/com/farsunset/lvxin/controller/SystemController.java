...
public class SystemController {
	...
	public int login(String password, String account, HttpServletRequest request) {

		Manager target = managerService.queryByAccount(account);
		if (target != null && DigestUtils.md5DigestAsHex(password.getBytes()).equals(target.getPassword())) {
	...
	public int modifyPassword(HttpServletRequest request, HttpServletResponse response) {
		...
		if (!manager.getPassword().equals(DigestUtils.md5DigestAsHex(oldPassword.getBytes()))) {
			return 403;
		}
		manager.setPassword(DigestUtils.md5DigestAsHex(newPassword.getBytes()));
