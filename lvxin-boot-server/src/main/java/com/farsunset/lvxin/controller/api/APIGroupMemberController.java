...
public class APIGroupMemberController {
	...
	public BaseResult list(long groupId) {
		...
		result.dataList = groupMemberService.getMemberList(groupId);
		return result;
	}
	...
	public BaseResult add(GroupMember groupMember) {
		...
		if (groupService.queryById(groupMember.getGroupId()) == null) {
			result.code = 404;
		} else {
	...
	public BaseResult getout(Long groupId, String account) {
		...
		List<String> list = new ArrayList<String>(Arrays.asList(accounts));
		groupMemberService.remove(list, groupId);
