...
import com.farsunset.lvxin.network.MessageDispatcher;
...
public class CIMMessagePusher {
	...
	public void push(com.farsunset.cim.sdk.server.model.Message msg, CIMSession session) {
		...
		if (session.isConnected() && !Objects.equals(host, session.getHost())) {
			messageDispatcher.forward(msg, session.getHost());
			return;
		}
		...
		if (Objects.equals(session.getApnsAble(), CIMSession.APNS_ON)) {
	 ...
	 * @param msg
	 */
	public void push(com.farsunset.lvxin.model.Message msg) {
		Message message = MessageUtil.transform(msg);
