var mLayim;
function initLayim(realPath) {
	window.localStorage.serverPath = realPath;
	layui.use('layim',function(layim) {
		...
		layim.config({
							...
							// 初始化接口
							init : {
								url : window.localStorage.serverPath+'/web/client/main.api',
								data : {
							...
							,
							members : {
								url : window.localStorage.serverPath+'/web/client/members.api',
								data : {}
							}
							// 扩展工具栏
							,
...
function notifyReceivedMessage(message){
	...
	if(message.format=='1'){
		...
		message.content = "img["+window.localStorage.serverPath+"/files/other-files/"+image.image+"]"
	}
	if(message.format=='2'){
		var voice = JSON.parse(message.content);
		message.content = "audio["+window.localStorage.serverPath+"/files/other-files/"+voice.key+"]"
	}
	if(message.format=='3'){
		var file = JSON.parse(message.content);
		message.content = "file("+window.localStorage.serverPath+"/files/other-files/"+file.key+")["+file.name+"]"
	}
	if(message.format=='8'){
		var video = JSON.parse(message.content);
		message.content = "video["+window.localStorage.serverPath+"/files/other-files/"+video.video+"]"
	}
	if(message.format=='4'){
	...
	var fromMessage = {
			  username: from.username //消息来源用户名
				  ,avatar: window.localStorage.serverPath+"/files/user-icon/"+from.id //消息来源用户头像
...
function sendMessage(data){
	...
	$.post(window.localStorage.serverPath +"/web/client/send.api", {content:content,action:action,sender:sender,receiver:receiver,format:'0'},
		function(data){
	});
}

function receiptMessage(mid){
	$.post(window.localStorage.serverPath +"/web/client/receipt.api", {mid:mid},
		function(data){
	});
}