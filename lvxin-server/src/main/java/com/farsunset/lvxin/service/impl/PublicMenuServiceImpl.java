...
public class PublicMenuServiceImpl implements PublicMenuService {
	...
	public PublicMenu queryById(String gid) {
		...
		return publicMenuRepository.get(gid);
	}
	...
	public void update(PublicMenu publicMenu) {
		...
		publicMenuRepository.merge(publicMenu);
	...
	public void add(PublicMenu publicMenu) {
		...
		if (publicMenu.getType() == PublicMenu.ACTION_ROOT
				&& publicMenuRepository.queryRootCount(publicMenu) >= Constants.MAX_PUB_ROOT_MENU) {
			throw new IllegalExceedRootLimitException();
		}
		if (publicMenu.getFid() != null && publicMenuRepository.queryChildCount(publicMenu) >= Constants.MAX_PUB_SUB_MENU) {
		...
		int sort = publicMenuRepository.getMaxSort(publicMenu) + 1;
		publicMenu.setSort(sort);
	...
	public void delete(PublicMenu publicMenu) {
		publicMenuRepository.deleteSubMenu(publicMenu.getGid());
		publicMenuRepository.delete(publicMenu);
	...
	private void boradcastOnMenuUpdated(String account) {
		...
		message.setContent(JSON.toJSONString(publicMenuRepository.getPublicMenuList(account)));
