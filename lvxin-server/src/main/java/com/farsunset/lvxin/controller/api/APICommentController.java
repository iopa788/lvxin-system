...
public class APICommentController {
	...
	public BaseResult publish(@TokenAccount String account,Comment comment, String replyAccount, String authorAccount) {
	...
	public BaseResult list(Comment comment, Page page) {

		BaseResult result = new BaseResult();

		page = this.commentService.queryByPage(comment, page);
	 ...
	 */
	public void broadcastDeleteMessage(String authorAccount, Comment comment) {
	 ...
	 * 有新的赞时，发送提醒消息文章发布者
	 */
	public void broadcastPraiseMessage(String authorAccount, Comment comment) {
		...
		if (isNotSelf) {
			...
			praiseMessage.setContent(JSON.toJSONString(comment));
	 ...
	 * 有新的评论时，发送提醒消息给 被回复者
	 */
	public void broadcastCommentMessage(String authorAccount, String replyAccount, Comment comment) {
		Message commentMessage = new Message();
		commentMessage.setSender(comment.getAccount());
		commentMessage.setContent(JSON.toJSONString(comment));
