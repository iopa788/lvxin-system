...
public class APIGroupController {
	...
	public BaseResult look() {
		...
		BaseResult result = new BaseResult();
		List<Group> groupList = groupServiceImpl.queryAll();
		result.dataList = groupList;
	...
	public BaseResult detailed(long groupId) {
		...
		result.data = groupServiceImpl.queryById(groupId);
	...
	public BaseResult update(Group group) {
		...
		Group target = groupServiceImpl.queryById(group.getGroupId());
		target.setName(group.getName());
		target.setSummary(group.getSummary());
		groupServiceImpl.update(target);
	...
	public BaseResult disband(Group group,@TokenAccount String founder) {
		BaseResult result = new BaseResult();
		group.setFounder(founder);
		groupServiceImpl.delete(group.getFounder(), group.getGroupId());
		return result;
	}
