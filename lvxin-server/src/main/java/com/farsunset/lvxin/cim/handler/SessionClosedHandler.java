...
public class SessionClosedHandler implements CIMRequestHandler {
	...
	public ReplyBody process(CIMSession ios, SentBody message) {
		...
		logger.warn("sessionClosed()... nid:" + ios.getNid() + " account:" + account + " isConnected:" + ios.isConnected());
	...
	public   void broadcastOfflineMessage(String account) {
		...
		ContextHolder.getBean(BroadcastMessagePusher.class).pushOnline(offlineMessage);
	}
