...
public class ModifyProfileHandler implements CIMRequestHandler {
	...
	protected final Logger logger = Logger.getLogger(ModifyProfileHandler.class);
	@Autowired
	...
	private void sendBroadcast(User user) {
		...
		updateMessage
				.setContent(new Action111Builder().buildJsonString(user.getAccount(), user.getName(), user.getMotto()));
